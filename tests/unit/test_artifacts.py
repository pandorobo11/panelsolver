...
class ArtifactProjectionTests(unittest.TestCase):
    ...
    def test_vtp_write_failures_preserve_existing_file_and_clean_temp(self) -> None:
        for target in ("pv.PolyData.save", "os.fsync", "os.replace"):
            with self.subTest(target=target), tempfile.TemporaryDirectory() as temp_dir:
                output = Path(temp_dir) / "case001.vtp"
                output.write_bytes(b"existing-vtp")
                with (
                    patch(
                        f"panelsolver.app.artifact_io.{target}",
