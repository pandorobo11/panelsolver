...
On macOS, use Python 3.12 or 3.13: the QTBUG-149612 workaround pins PySide6/Qt
6.9.0, whose [package metadata](https://pypi.org/pypi/PySide6/6.9.0/json)
declares `Requires-Python: >=3.9,<3.14`. Python 3.12 is the current CI baseline.
Windows/Linux retain the existing dependency range without this macOS-specific
upper limit; the project's `requires-python = ">=3.12"` remains unchanged.
...
A universal lockfile is not proof that every listed environment is installable
or supported. [uv's resolver](https://docs.astral.sh/uv/concepts/resolution/#universal-resolution)
ignores dependency `Requires-Python` upper bounds when resolving. In particular,
uv 0.9.13 can resolve and install PySide6 6.9.0 on Python 3.14 despite its declared
requirement, whereas `pip` rejects it. Always select Python 3.12 or 3.13 on
macOS; do not infer support from successful uv resolution or synchronization.
Keep the macOS Python 3.14/3.15 resolution markers generated by uv, rather than
hand-editing the lockfile or removing the Qt dependency for those interpreters.
Reassess the pin under the [native GUI validation criteria](gui-visual-smoke.md#macos-table-accessibility-dependency)
